...
class FED:
    ...
    def binary_pellets(self):
        ...
        if 'Binary_Pellets' in self.data.columns:
            return self.data['Binary_Pellets']

        bp = self.data['Pellet_Count'].diff().copy()
        bp.iloc[0] = self.data['Pellet_Count'].iloc[0]
    ...
    def _binary_poke_for_side(self, side):
        ...
        bp.iloc[0] = self.data[col].iloc[0]

        return bp

    def interpellet_intervals(self, check_concat=True):

        if 'Interpellet_Intervals' in self.data.columns:
            return self.data['Interpellet_Intervals']
    ...
    def correct_pokes(self):
        ...
        if 'Correct_Pokes' in self.data.columns:
            return self.data['Correct_Pokes']
